...
export default function RegisterScreen({ onSubmit }) {
  ...
  return (
    <div className="h-full w-full flex items-center justify-center px-4">
      <div className="w-full max-w-[360px] bg-stone-900/60 backdrop-blur-3xl border border-amber-400/15 rounded-[3rem] px-7 py-9 shadow-2xl shadow-amber-500/5">
        <div className="flex flex-col items-center mb-8">
          <motion.div
            className="w-12 h-12 rounded-full bg-amber-500/10 border border-amber-400/25 flex items-center justify-center mb-3"
          ...
          <p className="text-[0.65rem] text-amber-500/40 tracking-[0.2em] uppercase mt-1">Fill details to unlock</p>
        </div>
        ...
        <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
          {FIELDS.map(({ key, icon: Icon, placeholder, type, inputMode }, i) => (
            <motion.div key={key} initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 * i, duration: 0.35 }}>
              <div className={`flex items-center gap-3 border-b-2 pb-2.5 transition-colors duration-300 ${focused === key ? 'border-amber-400' : errors[key] ? 'border-red-400/50' : 'border-stone-700/50'}`}>
              ...
              {errors[key] && (
                <motion.p className="text-red-400/80 text-[0.65rem] mt-1 ml-8" initial={{ opacity: 0, y: -3 }} animate={{ opacity: 1, y: 0 }}>
                  {errors[key]}
                </motion.p>
              )}
            </motion.div>
          ...
          <motion.button
            type="submit"
            className="w-full bg-gradient-to-r from-amber-500 to-yellow-600 text-stone-900 py-4 rounded-full font-bold uppercase tracking-[0.35em] text-sm flex items-center justify-center gap-2 shadow-lg shadow-amber-600/25 mt-3"
